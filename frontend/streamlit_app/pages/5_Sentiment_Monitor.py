...
class SentimentMonitor:
    ...
    def fetch_price_data(
        ...
        try:
            ...
            df = pd.DataFrame({"time": date_range, "ticker": ticker, "close": prices})
    ...
    def detect_sentiment_divergence(
        self, sentiment_df: pd.DataFrame, price_df: pd.DataFrame, window: int = 5
    ) -> pd.DataFrame:
        """
        ...
        merged["divergence"] = ((merged["sentiment_trend"] > 0) & (merged["price_trend"] < 0)) | (
            (merged["sentiment_trend"] < 0) & (merged["price_trend"] > 0)
        )

        # Calculate divergence strength
        merged["divergence_strength"] = np.abs(merged["sentiment_trend"] * merged["price_trend"])
    ...
    for source in sources:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                line={"color": colors.get(source, "gray"), "width": 2},
            )
        )
...
def display_sentiment_metrics(metrics: dict):
    ...
    with col1:
        ...
        sentiment_color = (
            "🟢" if sentiment_value > 0.1 else "🔴" if sentiment_value < -0.1 else "🟡"
        )
        st.metric(
            "Average Sentiment",
    ...
    with col3:
        st.metric(
            "Neutral %", f"{metrics['neutral_ratio']:.1%}", help="Percentage of neutral sentiment"
        )
...
def main():
    ...
    ticker = st.sidebar.text_input(
        "Ticker Symbol", value="AAPL", help="Enter the stock ticker symbol to monitor"
    ).upper()
    ...
    with st.sidebar.expander("Advanced Options"):
        ...
        show_raw_data = st.checkbox(
            "Show Raw Data", value=False, help="Display raw sentiment data tables"
        )
    ...
    divergence_data = monitor.detect_sentiment_divergence(
        aggregate_sentiment, price_data, window=divergence_window
    )
    ...
    if not recent_divergences.empty:
        st.warning(
            f"⚠️ {len(recent_divergences)} divergence signals detected in the selected period"
        )

        with st.expander("View Divergence Details"):
            st.dataframe(
                recent_divergences[
                    ["time", "sentiment_trend", "price_trend", "divergence_strength"]
                ].style.format(
                    {
                        "sentiment_trend": "{:.4f}",
                        "price_trend": "{:.4f}",

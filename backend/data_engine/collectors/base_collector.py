...
class BaseDataCollector(ABC):
    ...
    def __init__(self, name: str, rate_limit: int = 100):
        ...
        logger.info(
            f"Initialized {name} collector with rate limit {rate_limit}/min"
        )
    ...
    async def _check_rate_limit(self):
        ...
        if len(self._request_timestamps) >= self.rate_limit:
            wait_time = (
                self._request_timestamps[0] - minute_ago
            ).total_seconds()
            logger.warning(
        ...
        **kwargs,
        ...
        try:
            ...
            if data is None or data.height == 0:
                logger.warning(
                    f"No data collected for {ticker} from {self.name}"
                )
                return None
            ...
            if not is_valid:
                logger.error(
                    f"Data validation failed for {ticker} from {self.name}"
                )
                return None

            logger.success(
                f"Successfully collected {data.height} rows for {ticker}"
            )
            return data

        except Exception as e:
            logger.error(
                f"Error collecting data for {ticker} from {self.name}: {e}"
            )
            raise

...
SELECT create_hypertable (
        'price_data', 'time', if_not_exists => TRUE
    );
...
CREATE INDEX IF NOT EXISTS idx_features_ticker ON features (
    ticker,
    feature_name,
    time DESC
);
...
SELECT create_hypertable (
        'sentiment_data', 'time', if_not_exists => TRUE
    );
...
SELECT create_hypertable (
        'predictions', 'prediction_time', if_not_exists => TRUE
    );

CREATE INDEX IF NOT EXISTS idx_predictions_ticker ON predictions (ticker, prediction_time DESC);

CREATE INDEX IF NOT EXISTS idx_predictions_model ON predictions (
    model_name,
...
SELECT create_hypertable (
        'market_regimes', 'time', if_not_exists => TRUE
    );
...
CREATE INDEX IF NOT EXISTS idx_backtest_strategy ON backtest_results (
    strategy_name,
    created_at DESC
);

...
def update_ticker_data(
    ...
    try:
        ...
        price_count = loop.run_until_complete(bulk_insert_price_data(price_data))
...
def update_all_features(
    ...
    try:
        ...
        logger.success(f"✅ Feature update complete: {successful}/{len(tickers)}")
        return summary
...
def health_check_task() -> Dict[str, Any]:
    ...
    try:
        ...
        # Check database
        try:
            from db.models import check_db_connection

            db_ok = loop.run_until_complete(check_db_connection())
            health_status["database"] = "healthy" if db_ok else "unhealthy"
        except Exception as e:
            logger.error(f"Database check failed: {e}")
            health_status["database"] = "unhealthy"

        # Check data freshness (most recent data should be withing 24 hours)
        try:
            latest = loop.run_until_complete(get_latest_price("AAPL"))
            if latest:
        ...
        try:
            collector = YFinanceCollector()
            collector_health = loop.run_until_complete(collector.health_check())
...
def cleanup_old_results(days_to_keep: int = 90) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            loop.run_until_complete(execute_raw_sql("VACUUM ANALYZE"))

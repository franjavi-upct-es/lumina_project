...
def _calculate_monte_carlo_var(returns: pd.Series, confidence_level: float, n_simulations: int = 10000) -> tuple:
...
async def stress_test(request: StressTestRequest):
    ...
    try:
        ...
        # Current exposure
        current_vol = float(np.sqrt(np.dot(weights.T, np.dot(returns_df.cov(), weights))) * np.sqrt(252))
        ...
        for i, (dd, cum_val) in enumerate(zip(drawdown, cumulative)):
            if dd < 0 and not in_drawdown:
                ...
                drawdown_periods.append({
                    "start_date": returns_df.index[start_idx].isoformat() if hasattr(returns_df.index[start_idx],
                                                                                     'isoformat') else str(start_idx),
                    "end_date": returns_df.index[i].isoformat() if hasattr(returns_df.index[i], 'isoformat') else str(
                        i),
        ...
        if request.rolling_window:
            ...
            for i in range(request.rolling_window, len(returns_df)):
                ...
                rolling_corr_data.append({
                    "date": returns_df.index[i].isoformat() if hasattr(returns_df.index[i], 'isoformat') else str(i),
...
async def analyze_liquidity_risk(
    ...
    try:
        ...
        return {
            ...
            "liquidity_rating": (
                "High" if liquidity_score > 70 else
                "Medium" if liquidity_score > 40 else
                "Low"
            ),
            "warnings": (
                ["Low liquidity - may face slippage"] if liquidity_score < 40 else
                ["Consider market impact"] if liquidity_score < 70 else
                []
            )
        }
...
async def scenario_analysis(
    ...
    try:
        ...
        current_vol = float(np.sqrt(np.dot(weights_array.T, np.dot(returns_df.cov() * 252, weights_array))))
    ...
    try:
        ...
        try:
            ...
            if spy_data and spy_data.height > 0:
                ...
                covariance = np.cov(portfolio_returns[-common_len:], spy_returns[-common_len:])[0, 1]
        ...
        return {
            "summary": {
                ...
                "risk_level": (
                    "Low" if annual_vol < 0.15 else
                    "Medium" if annual_vol < 0.25 else
                    "High"
                )
            },
            "value_at_risk": {
...
def _generate_risk_warnings(volatility: float, max_dd: float, sharpe: float, correlation: float) -> List[str]:
